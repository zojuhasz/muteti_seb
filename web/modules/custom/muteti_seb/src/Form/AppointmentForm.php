...
final class AppointmentForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?string $date=NULL, ?string $slot=NULL): array {
    $this->appointment=$this->database->select('muteti_appointment','a')->fields('a')->condition('department','Sebészet')->condition('admission_date',$date)->condition('slot_type',$slot)->execute()->fetchObject() ?: NULL;
    ...
    $doctors=['0'=>'-']+$this->database->select('muteti_doctor','d')->fields('d',['id','name'])->condition('department','Sebészet')->condition('active',1)->orderBy('name')->execute()->fetchAllKeyed();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    else { $fields += ['department'=>'Sebészet','admission_date'=>$v['date'],'slot_type'=>$v['slot'],'created_by'=>(int)$this->currentUser()->id(),'created'=>$fields['changed']]; $this->database->insert('muteti_appointment')->fields($fields)->execute(); }
